...
const FormProposta: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://slug-liberal-wallaby.ngrok-free.app:5000/drones/${userId}`);
        if (!response.ok) {
  ...
  const onSubmit = async (data: PropostaForm) => {
    try {
      const response = await fetch('https://slug-liberal-wallaby.ngrok-free.app:5000/proposta', {
        method: "POST",
        headers: {
