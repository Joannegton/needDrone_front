...
const FormProposta: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://18.188.189.201:5000/projeto/${projectId}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://18.188.189.201:5000/drones/${userId}`);
        if (!response.ok) {
  ...
  const onSubmit = async (data: PropostaForm) => {
    try {
      const response = await fetch('http://18.188.189.201:5000/proposta', {
        method: "POST",
        headers: {
