...
const FormAtualizacaoClient = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://needdrone.onrender.com/cliente/${userId}`);
        if (!response.ok) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const response = await fetch(`https://needdrone.onrender.com/cliente/atualizar/${userId}`, {
        method: 'PUT',
